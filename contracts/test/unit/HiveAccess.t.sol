...
contract HiveAccessTest is Test {
    ...
    function test_calculateTier_none() public view {
        assertEq(uint256(access.calculateTier(0, 0)), uint256(HiveTypes.Tier.NONE));
        assertEq(uint256(access.calculateTier(9, 100e18)), uint256(HiveTypes.Tier.NONE));
        assertEq(uint256(access.calculateTier(10, 99e18)), uint256(HiveTypes.Tier.NONE));
    }

    function test_calculateTier_bronze() public view {
        assertEq(uint256(access.calculateTier(10, 100e18)), uint256(HiveTypes.Tier.BRONZE));
        assertEq(uint256(access.calculateTier(29, 499e18)), uint256(HiveTypes.Tier.BRONZE));
    }

    function test_calculateTier_silver() public view {
        assertEq(uint256(access.calculateTier(30, 500e18)), uint256(HiveTypes.Tier.SILVER));
    }

    function test_calculateTier_gold() public view {
        assertEq(uint256(access.calculateTier(60, 2000e18)), uint256(HiveTypes.Tier.GOLD));
    }

    function test_calculateTier_diamond() public view {
        assertEq(uint256(access.calculateTier(100, 5000e18)), uint256(HiveTypes.Tier.DIAMOND));
    }
    ...
    function testFuzz_calculateTier_monotonic(uint256 rep, uint256 stake) public view {
        rep = bound(rep, 0, 200);
        stake = bound(stake, 0, 10000e18);

        HiveTypes.Tier tier = access.calculateTier(rep, stake);
